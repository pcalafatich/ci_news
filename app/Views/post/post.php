<div id="wrapper">
    <div class="container">
        <div class="row">
            ...
            <div id="content" class="<?= $post->show_right_column == 1 ? 'col-sm-8' : 'col-sm-12'; ?> col-xs-12">
                <div class="post-content">
                    ...
                    <div class="post-share">
                        <?= view('post/_post_share'); ?>
                    </div>
                    <?php if ($post->post_type == 'video'):
                        echo view('post/details/_video', ['post' => $post]);
                    elseif ($post->post_type == 'audio'):
                        echo view('post/details/_audio', ['post' => $post]);
                    elseif ($post->post_type == 'gallery'):
                        echo view('post/details/_gallery', ['post' => $post]);
                    elseif ($post->post_type == 'sorted_list'):
                        echo view('post/details/_sorted_list', ['post' => $post]);
                    elseif ($post->post_type == 'trivia_quiz' || $post->post_type == 'personality_quiz'):
                        echo view('post/details/_quiz', ['post' => $post]);
                    else:
                        echo view('post/details/_article', ['post' => $post]);
                    endif;
                    echo view('partials/_ad_spaces', ['adSpace' => 'post_top', 'class' => 'bn-p-t-20']); ?>
                    <div class="post-text">
                        <?= $post->content; ?>
                    </div>
                        ...
                        <div class="post-files">
                            ...
                            <?php foreach ($files as $file): ?>
                                <form action="<?= base_url('download-file'); ?>" method="post">
                                    <?= csrf_field(); ?>
                                    <input type="hidden" name="storage" value="<?= esc($file->storage); ?>">
                                    <div class="file">
                                        <button type="submit" name="path" value="<?= esc($file->file_path); ?>"><i class="icon-file"></i><?= esc($file->file_name); ?></button>
                                    </div>
                                </form>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div class="row">
                            <div class="reactions noselect">
                                ...
                                <div id="reactions_result">
                                    <?= view('post/_emoji_reactions', ['reactions' => $reactions]); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- PUBLICIDAD -->
                echo view('partials/_ad_spaces', ['adSpace' => 'post_bottom', 'class' => 'bn-p-b']);
                <!-- PUBLICIDAD -->

                <!-- MOSTRAR AUTOR -->
                if ($generalSettings->show_post_author == 1):
                    echo view('post/_post_about_author', ['postUser' => $postUser]);
                endif; ?>
                <!-- FIN MOSTRAR AUTOR -->
                        ...
                        <div class="row">
                            ...
                            foreach ($relatedPosts as $item):
                                ...
                                <div class="col-sm-4 col-xs-12">
                                    <?= view("post/_post_item_mid", ['post' => $item]); ?>
                                </div>
                                <?php $i++;
                            endforeach; ?>
                        </div>
                                        ...
                                        <div class="tab-content">
                                            <?php if ($generalSettings->comment_system == 1): ?>
                                                <div id="site_comments" class="tab-pane fade in active">
                                                    <?= view('post/_add_comment', ['post' => $post, 'commentCount' => $post->comment_count]); ?>
                                                    <div id="comment-result">
                                                        <?= view('post/_comments', ['post' => $post, 'commentCount' => $post->comment_count]); ?>
                                                    </div>
                                                </div>
                                            <?php endif;
...
<?php endif; ?>
<!-- FIN SISTEMA DE COMENTARIOS DE FACEBOOK -->